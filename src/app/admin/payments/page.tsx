...
import { useDailyPaymentStats, usePaymentHistory, useAllSellers, useCreateRefund } from '@/lib/supabase/hooks'
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <div className="card-glass rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-white">
            {statsLoading ? '...' : `$${stats.cashTotal.toFixed(2)}`}
          </p>
        </div>
        ...
        <div className="card-glass rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-white">
            {statsLoading ? '...' : `$${stats.cardTotal.toFixed(2)}`}
          </p>
        </div>
        ...
        <div className="card-glass rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-white">
            {statsLoading ? '...' : `$${stats.tipsTotal.toFixed(2)}`}
          </p>
        </div>
      </motion.div>
          ...
          <div className="flex items-center gap-3">
            ...
            {stats.sellerStats.length > 0 && (
              <span className="text-sm text-[var(--muted-foreground)]">
                Top: {stats.sellerStats[0]?.seller_name} (${stats.sellerStats[0]?.tips.toFixed(2)} tips)
              </span>
            )}
          </div>
          {showSellerStats ? (
                ...
                <tbody>
                  {stats.sellerStats.map((seller, idx) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-right text-[var(--foreground)]">${seller.revenue.toFixed(2)}</td>
                      <td className="p-4 text-right">
                        <span className="text-purple-400 font-semibold">${seller.tips.toFixed(2)}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                {payments.map((payment) => (
                  <tr key={payment.id} className="border-b border-[var(--card-border)]/50 hover:bg-[var(--muted)]/20">
                    ...
                    <td className="p-4 text-right font-semibold text-white">
                      ${(Number(payment.amount) + Number(payment.tip_amount || 0)).toFixed(2)}
                    </td>
