...
export default function SellerTablesPage() {
  ...
  const { tables, loading: isLoading, refresh } = useTables();
  const { create, loading: isCreating } = useCreateTable();

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [newTableNumber, setNewTableNumber] = useState("");
  const [newTableSection, setNewTableSection] = useState("Inside");
  ...
  const sections = ["All", "Inside", "Bar", "Patio"];
  ...
  return (
    <div className="flex-1 flex flex-col px-6 py-8">
      ...
      >
        ...
        {/* Create Table Button - Only for admin */}
        {seller && (
          <motion.div
